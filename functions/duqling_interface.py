...
class DuqlingInterface:
    ...
    def generate_data(self, func_name:str, n_samples:int, seed:Optional[int]=42, **kwargs) -> Tuple[np.array, np.array]:
        """
        ...
        try:
            if kwargs:
                result = self.gen_func(func_name, n_samples=n_samples, seed=seed, **kwargs)
            else:
                result = self.gen_func(func_name, n_samples=n_samples, seed=seed)
